...
|**1.2**|2022-07-06|Version 1.2 of the API supports sending **PATCH** on **/transfers/**_{ID}_**/error** from the Switch in case the Payee FSP is using PUT /transfers/{ID} with transferState COMMITTED and the commit failed in the Switch. There's no change for PUT /transfers/{ID} with RESERVED as the Payee FSP is expecting a PATCH /transfers/{ID} with either COMMITTED or ABORTED in version 1.1. (Version 2.0 of the API adds same possibility of sending **PATCH** on **/transfers/**_{ID}_**/error** from the Switch in case the Payee FSP is using PUT /transfers/{ID} with RESERVED and there is an error. This is based on the acceptance of the change request: https://github.com/mojaloop/mojaloop-specification/issues/103|)
...
| **1.2** | 2022-01-31 | The resource is extended to support error notifications for failures, using the HTTP Method **PATCH** introduced in version **1.1**. This can be used when there is an internal validation or processing error (not schema validation) during handling of the **PUT /transfers/**_{ID}_ sent from the Payee FSP, when COMMITTED is used as transferState. The new error callback **PATCH /transfers/{ID}/error** is described in Section 6.7.5.2. <br><br> This is based on the acceptance of the change request: https://github.com/mojaloop/mojaloop-specification/issues/103|
...
[Figure 56a](#figure-56a) shows an example in which the validation in the Switch failed due to some reason, for example the a business rule validation caused a failure in the Switch. This is the same example as in [Figure 53](#figure-53), but the Payee FSP receives a failure notification before performing the actual transfer to the Payee so that the Payee FSP can take remedial action and helps decrease the need for reconciliation between the Switch and the Payee FSP.

###### Figure 56a

{% uml src="assets/diagrams/sequence/figure56a.plantuml" %}
{% enduml %}

**Figure 56a -- Commit notification where commit of transfer in Switch failed**
...
If the server is unable to commit a transfer, or another processing or validation error occurs, during the processing of a **PUT /transfers/**_{ID}_ callback with transferState as COMMITTED, the notification **PATCH** **/transfers/**_{ID}_**/error** is used. The _{ID}_ in the URI should contain the **transferId** (see [Table 30](#table-30)) that was used for the creation of the transfer. See [Table 34](#table-34) for data model.
